...
class DataManipulation{
    ...
    func retrieveData(){ //fetch data from core data
        ...
        //sort data by
        fetchRequest.sortDescriptors = [NSSortDescriptor.init(key: "dataIndex", ascending: true)]

        // harus menggunakan do catch
        do {
            let result = try manageContext.fetch(fetchRequest)
            for data in result as! [NSManagedObject]{
                model.append(AnalyzeMovementData(dataIndex: data.value(forKey: "dataIndex") as! Int, recordDate: data.value(forKey: "analyzeDate") as! String, movementIconName: data.value(forKey: "movementImageName") as! String, movementName: data.value(forKey: "movementName") as! String, movementAccuracy: data.value(forKey: "accuracy") as! String))
            }
        } catch  let error as NSError{
